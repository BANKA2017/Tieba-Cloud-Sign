...
if (!empty($i['user']['bduss'])) {
    ...
    if(!empty($f)) {
        ...
        foreach($i['user']['baidu'] as $pkey => $pval) {
            echo '<li role="presentation" class="pidlist"><a data-toggle="tab" onclick="showtbpanel(\''.$pkey.'\');">'.($pval ?: $i['user']['baidu_name_show'][$pkey] ?: $i['user']['baidu_portrait'][$pkey]).'</a></li>';
        }
        echo '</ul>';
        echo '<form action="setting.php?mod=showtb&set" method="post">';
        foreach($i['user']['baidu_name_show'] as $pkey => $pval) {
            if(isset($f[$pkey])) {
                    ...
                    <div class="input-group">
                        ...
                        <select name="pid" required class="form-control">
                            <?php foreach ($i['user']['bduss'] as $key => $value) {
                                $name = empty($i['user']['baidu'][$key] . $i['user']['baidu_name_show'][$key]) ? ' [未知]' : (' ('.$i['user']['baidu'][$key] . " [{$i['user']['baidu_name_show'][$key]}]".')');
